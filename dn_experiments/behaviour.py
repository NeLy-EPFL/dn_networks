...
"""
beh_mapping = [ 'background', 'walk', 'rest', 'back', 'groom', 'leg rub', 'posterior']
collapse_groom = True
...
beh_cmaplist = [myplt.WHITE, myplt.DARKGREEN, myplt.DARKBLUE, myplt.DARKCYAN, myplt.DARKRED, myplt.DARKRED, myplt.DARKPINK]

beh_cmap = mpl.colors.LinearSegmentedColormap.from_list(
    name='behaviour', colors=beh_cmaplist, N=256)  # , N=6, )
...
def get_beh_info_in_twop_df(beh_df, twop_df, keys=["silent", "fast", "me_all"],
    ...
    for trial_name in np.unique(beh_df.index.get_level_values("TrialName")):
        ...
        for key, reduce_fn in zip(keys, reduce_fns):
            signals_2p.append(synchronisation.reduce_during_2p_frame(beh_df.loc[beh_df_index,"twop_index"].values,
        ...
        new_twop_df_index = np.logical_and.reduce((twop_df.index.get_level_values("Frame").values >= min_index,
...
def get_natural_beh_starts_trial(beh_df, beh="back", trial_name="", remove_laser=True, return_beh_prob=True, t_win=[-5,10], min_dur=params.backwalk_min_dur, min_dist=params.backwalk_min_dist):
    ...
    if "back" in beh_df.keys() and "walk" in beh_df.keys() and "rest" in beh_df.keys() and "groom" in beh_df.keys() and "frub" in beh_df.keys():
        ...
        walk_beh, rest_beh, back_beh, groom_beh, frub_beh, post_beh = discriminate_beh(beh_df)
    ...
    if beh == "back":
        ...
        raise NotImplementedError(f"Warning: behaviour of interest must be 'walk', 'back', 'groom', or 'rest', but was {beh}.")
    ...
    if remove_laser and "laser_stim" in beh_df.keys():
        ...
        for i_s, (start, stop) in enumerate(zip(beh_starts, beh_stops)):
            if not beh_df.laser_stim.values[start] and not beh_df.laser_stim.values[stop]:
                to_keep[i_s] = True
    ...
    if beh == "back":
        beh_event_filter_strict = np.logical_and.reduce((beh_pre == 0, beh_dur >= min_dur, beh_cum <= min_dist))
    elif beh == "walk":
        beh_event_filter_strict = np.logical_and.reduce((beh_pre == 0, beh_dur >= min_dur, beh_cum >= min_dist))
    elif beh == "rest" or beh == "groom":
        beh_event_filter_strict = np.logical_and(beh_pre == 0, beh_dur >= min_dur)
    ...
    beh_starts_filt_strict = beh_starts_filt_strict[twop_starts_filt_strict >= -1*params.fs_int*t_win[0]+4]
    beh_stops_filt_strict = beh_stops_filt_strict[twop_starts_filt_strict >= -1*params.fs_int*t_win[0]+4]
    twop_starts_filt_strict = twop_starts_filt_strict[twop_starts_filt_strict >= -1*params.fs_int*t_win[0]+4]

    twop_starts_filt_strict = twop_starts_filt_strict[twop_stops_filt_strict <= max_twop_index-params.fs_int*t_win[1]-4]
    beh_starts_filt_strict = beh_starts_filt_strict[twop_stops_filt_strict <= max_twop_index-params.fs_int*t_win[1]-4]
    beh_stops_filt_strict = beh_stops_filt_strict[twop_stops_filt_strict <= max_twop_index-params.fs_int*t_win[1]-4]
    twop_stops_filt_strict = twop_stops_filt_strict[twop_stops_filt_strict <= max_twop_index-params.fs_int*t_win[1]-4]


    if return_beh_prob and len(twop_stops_filt_strict):
        i_win = (fs_beh * np.array(t_win)).astype(int)
        beh_df_aligned = beh_df[beh_starts_filt_strict[0]+i_win[0]:beh_starts_filt_strict[0]+i_win[1]][["t","twop_index"]]
        beh_df_aligned["t"] -= beh_df_aligned.iloc[-i_win[0]]["t"]
        beh_df_aligned["twop_index"] -= int(beh_df_aligned.iloc[-i_win[0]]["twop_index"])
        ...
        for start in beh_starts_filt_strict:
            beh_df_aligned["back"] += back_beh[start+i_win[0]:start+i_win[1]]
            beh_df_aligned["rest"] += rest_beh[start+i_win[0]:start+i_win[1]]
            beh_df_aligned["walk"] += walk_beh[start+i_win[0]:start+i_win[1]]
            beh_df_aligned["v_forw"] += v_forw_beh_filt[start+i_win[0]:start+i_win[1]]

    elif not len(twop_stops_filt_strict):
        beh_df_aligned = None
    if return_beh_prob:
        return twop_starts_filt_strict, twop_stops_filt_strict, beh_starts_filt_strict, beh_stops_filt_strict, beh_df_aligned
    else:
...
def get_beh_trigger_into_dfs(twop_df, beh_df, beh="back"):
    if beh == "back":
        ...
        raise NotImplementedError(f"Warning: behaviour of interest must be 'walk', 'back', or 'rest', but was {beh}.")
    
    if twop_df is not None:
        twop_df.insert(loc=len(twop_df.columns), column=f"{beh}_trig_start", value=np.zeros((len(twop_df)), dtype=bool))
        twop_df.insert(loc=len(twop_df.columns), column=f"{beh}_trig_stop", value=np.zeros((len(twop_df)), dtype=bool))
        twop_df.insert(loc=len(twop_df.columns), column=f"{beh}_trig", value=np.zeros((len(twop_df)), dtype=bool))
    beh_df.insert(loc=len(beh_df.columns), column=f"{beh}_trig_start", value=np.zeros((len(beh_df)), dtype=bool))
    beh_df.insert(loc=len(beh_df.columns), column=f"{beh}_trig_stop", value=np.zeros((len(beh_df)), dtype=bool))
    beh_df.insert(loc=len(beh_df.columns), column=f"{beh}_trig", value=np.zeros((len(beh_df)), dtype=bool))
    ...
    for trial_name in np.unique(beh_df.index.get_level_values("TrialName")):
        ...
        starts, stops, beh_starts, beh_stops = get_natural_beh_starts_trial(beh_df.loc[beh_df_index], beh=beh,
                                                                            remove_laser=True, return_beh_prob=False,
                                                                            t_win=[-5,10], min_dur=min_dur, min_dist=min_dist)
        for start, stop, beh_start, beh_stop in zip(starts, stops, beh_starts, beh_stops):
            new_beh_df_index_start = np.logical_and(beh_df_index, beh_df.index.get_level_values("Frame") == beh_start)
            beh_df.loc[new_beh_df_index_start, f"{beh}_trig_start"] = True
            new_beh_df_index_stop = np.logical_and(beh_df_index, beh_df.index.get_level_values("Frame") == beh_stop)
            ...
            beh_df.loc[new_beh_df_index_start:new_beh_df_index_stop, f"{beh}_trig"] = True
            
            if twop_df is not None:
                new_twop_df_index_start = np.logical_and(twop_df_index, twop_df.index.get_level_values("Frame") == start)
                twop_df.loc[new_twop_df_index_start, f"{beh}_trig_start"] = True
                new_twop_df_index_stop = np.logical_and(twop_df_index, twop_df.index.get_level_values("Frame") == stop)
                twop_df.loc[new_twop_df_index_stop, f"{beh}_trig_stop"] = True
                new_twop_df_index_start = np.argwhere(new_twop_df_index_start)[0][0]
                new_twop_df_index_stop = np.argwhere(new_twop_df_index_stop)[0][0]
...
def discriminate_beh(beh_df=None, v=None, v_forw=None, me_q=None, mef_q=None, mem_q=None, meb_q=None, method=params.beh_class_method):
    ...
    if method == "motionenergy":
        ...
        groom = np.logical_and(get_groom_me(v, v_forw, me_q, mef_q, mem_q, meb_q), np.logical_not(rest))
        frub = np.logical_and(get_frub_me(v, v_forw, me_q, mef_q, mem_q, meb_q), np.logical_not(rest))
        post = np.logical_and(get_post_me(v, v_forw, me_q, mef_q, mem_q, meb_q), np.logical_not(rest))
    elif method == "sleap":
        ...
        rest = get_rest_sleap(v, v_forw, frleg_height, mef_tita, mem_tita, meh_tita)
        walk = get_walk_sleap(v, v_forw, frleg_height, mef_tita, mem_tita, meh_tita)
        back = get_back_sleap(v, v_forw, frleg_height, mef_tita, mem_tita, meh_tita)
        groom = get_groom_sleap(v, v_forw, frleg_height, mef_tita, mem_tita, meh_tita)
        frub = get_frub_sleap(v, v_forw, frleg_height, mef_tita, mem_tita, meh_tita)
        post = get_post_sleap(v, v_forw, frleg_height, mef_tita, mem_tita, meh_tita)
    else:
...
def get_rest_me(v, v_forw, me_q, mef_q, mem_q, meb_q, thr=[1, 1, 0.33, 0.33, -1*np.inf, -1*np.inf],
             sign=[np.less_equal, np.less_equal, np.less_equal, np.less_equal, np.greater, np.greater]):
    return get_beh_me(v, v_forw, me_q, mef_q, mem_q, meb_q, thr, sign)

def get_walk_me(v, v_forw, me_q, mef_q, mem_q, meb_q, thr=[1, 1, -1*np.inf, -1*np.inf, -1*np.inf, -1*np.inf],
             sign=[np.greater, np.greater, np.greater, np.greater, np.greater, np.greater]):
    return get_beh_me(v, v_forw, me_q, mef_q, mem_q, meb_q, thr, sign)

def get_back_me(v, v_forw, me_q, mef_q, mem_q, meb_q, thr=[1, -1, -1*np.inf, -1*np.inf, -1*np.inf, -1*np.inf],
             sign=[np.greater, np.less_equal, np.greater, np.greater, np.greater, np.greater]):
    return get_beh_me(v, v_forw, me_q, mef_q, mem_q, meb_q, thr, sign)

def get_groom_me(v, v_forw, me_q, mef_q, mem_q, meb_q, thr=[1, 1, np.inf, 0.33, 0.33, np.inf],
             sign=[np.less_equal, np.less_equal, np.less_equal, np.greater, np.less_equal, np.less_equal]):
    return get_beh_me(v, v_forw, me_q, mef_q, mem_q, meb_q, thr, sign)

def get_frub_me(v, v_forw, me_q, mef_q, mem_q, meb_q, thr=[1, 1, np.inf, 0.33, 0.33, np.inf],
             sign=[np.less_equal, np.less_equal, np.less_equal, np.less_equal, np.greater, np.less_equal]):
    return get_beh_me(v, v_forw, me_q, mef_q, mem_q, meb_q, thr, sign)

def get_post_me(v, v_forw, me_q, mef_q, mem_q, meb_q, thr=[1, 1, np.inf, 0.33, 0.33, 0.33],
             sign=[np.less_equal, np.less_equal, np.less_equal, np.less_equal, np.less_equal, np.greater]):
    return get_beh_me(v, v_forw, me_q, mef_q, mem_q, meb_q, thr, sign)
...
def get_rest_sleap(v, v_forw, frleg_height, mef_tita, mem_tita, meh_tita, thr=[0.75, 0.75, 0, 0.75, 0.75, 0.75],
             sign=[np.less_equal, np.less_equal, np.greater, np.less_equal, np.less_equal, np.less_equal]):
    return get_beh_sleap(v, v_forw, frleg_height, mef_tita, mem_tita, meh_tita, thr, sign)

def get_walk_sleap(v, v_forw, frleg_height, mef_tita, mem_tita, meh_tita, thr=[1, 1, -1*np.inf, -1*np.inf, -1*np.inf, -1*np.inf],
             sign=[np.greater, np.greater, np.greater, np.greater, np.greater, np.greater]):
    return get_beh_sleap(v, v_forw, frleg_height, mef_tita, mem_tita, meh_tita, thr, sign)

def get_back_sleap(v, v_forw, frleg_height, mef_tita, mem_tita, meh_tita, thr=[1, -1, -1*np.inf, -1*np.inf, -1*np.inf, -1*np.inf],
             sign=[np.greater, np.less_equal, np.greater, np.greater, np.greater, np.greater]):
    return get_beh_sleap(v, v_forw, frleg_height, mef_tita, mem_tita, meh_tita, thr, sign)

def get_groom_sleap(v, v_forw, frleg_height, mef_tita, mem_tita, meh_tita, thr=[0.75, 0.75, 0, np.inf, 0.75, 0.75],
             sign=[np.less_equal, np.less_equal, np.less_equal, np.less_equal, np.less_equal, np.less_equal]):
    return get_beh_sleap(v, v_forw, frleg_height, mef_tita, mem_tita, meh_tita, thr, sign)

def get_frub_sleap(v, v_forw, frleg_height, mef_tita, mem_tita, meh_tita, thr=[0.75, 0.75, 0, 0.75, 0.75, 0.75],
             sign=[np.less_equal, np.less_equal, np.greater, np.greater, np.less_equal, np.less_equal]):
    return get_beh_sleap(v, v_forw, frleg_height, mef_tita, mem_tita, meh_tita, thr, sign)

def get_post_sleap(v, v_forw, frleg_height, mef_tita, mem_tita, meh_tita, thr=[0.75, 0.75, 0, 0.75, np.inf, 0.75],
             sign=[np.less_equal, np.less_equal, np.greater, np.less_equal, np.less_equal, np.greater]):
    return get_beh_sleap(v, v_forw, frleg_height, mef_tita, mem_tita, meh_tita, thr, sign)
...
def add_beh_class_to_dfs(twop_df, beh_df):
    if twop_df is not None:
        ...
        if not "rest" in twop_df.keys():
            twop_df.insert(loc=len(twop_df.columns), column="rest", value=np.zeros((len(twop_df)), dtype=bool))
        if not "back" in twop_df.keys():
            twop_df.insert(loc=len(twop_df.columns), column="back", value=np.zeros((len(twop_df)), dtype=bool))
        twop_df.insert(loc=len(twop_df.columns), column="groom", value=np.zeros((len(twop_df)), dtype=bool))
        twop_df.insert(loc=len(twop_df.columns), column="frub", value=np.zeros((len(twop_df)), dtype=bool))
        twop_df.insert(loc=len(twop_df.columns), column="post", value=np.zeros((len(twop_df)), dtype=bool))
        twop_df.insert(loc=len(twop_df.columns), column="beh_class", value=np.zeros((len(twop_df)), dtype=int))
    if not "walk" in beh_df.keys():
        beh_df.insert(loc=len(beh_df.columns), column="walk", value=np.zeros((len(beh_df)), dtype=bool))
    if not "rest" in beh_df.keys():
        beh_df.insert(loc=len(beh_df.columns), column="rest", value=np.zeros((len(beh_df)), dtype=bool))
    if not "back" in beh_df.keys():
        beh_df.insert(loc=len(beh_df.columns), column="back", value=np.zeros((len(beh_df)), dtype=bool))
    beh_df.insert(loc=len(beh_df.columns), column="groom", value=np.zeros((len(beh_df)), dtype=bool))
    beh_df.insert(loc=len(beh_df.columns), column="frub", value=np.zeros((len(beh_df)), dtype=bool))
    beh_df.insert(loc=len(beh_df.columns), column="post", value=np.zeros((len(beh_df)), dtype=bool))
    beh_df.insert(loc=len(beh_df.columns), column="beh_class", value=np.zeros((len(beh_df)), dtype=int))
    ...
    for trial_name in np.unique(beh_df.index.get_level_values("TrialName")):
        ...
        if twop_df is not None:
            beh_class_2p = synchronisation.reduce_during_2p_frame(twop_index=beh_df.loc[beh_df_index, "twop_index"].values,
                                                        values=beh_class,
...
def get_pre_stim_beh(beh_df, trigger="laser_start", stim_p=[10,20], n_pre_stim=100, trials=None, return_starts=False):  # TODO: look at behavioural classification instead
    ...
    if trials is not None:
        beh_starts = [start for start in beh_starts
                  if any([beh_df.iloc[start].name[3].startswith(trial[:3])
                          for trial in trials])]
    if "laser" in trigger and stim_p is not None:
        try:
            beh_starts = [start for start in beh_starts \
                    if any([int(beh_df.iloc[start+fs_int].laser_power_uW) == p for p in stim_p])]
        except:
            try:
                print(f"will look at laser_power_mW instead of laser_power_uW. unique values are: {np.unique(twop_df.laser_power_mW)}")
                beh_starts = [start for start in beh_starts \
                        if any([int(beh_df.iloc[start+fs_int].laser_power_mW) == p for p in stim_p])]
            except:
                print(f"WARNING: laser_power_mW and laser_power_uW were not found. Will use laser_power.")
                stim_power_uW = stimulation.get_stim_p_uW(beh_df.laser_power.values)
                print(f"Converted to uW: {np.unique(stim_power_uW)}")
                beh_starts = [start for start in beh_starts \
                        if any([int(stim_power_uW[start+fs_int]) == p for p in stim_p])]

    # make sure that the ones that might be too early or too late are ignored.
    beh_starts = [start for start in beh_starts
                  if all([start - params.response_t_params_beh[0] > 0,
    ...
    for i_s, beh_start in enumerate(beh_starts):
        walk_pre[i_s], rest_pre[i_s] = discriminate_walk_rest_pre_stim(
            ...
            me_q=np.mean(beh_df.me_all_q.iloc[beh_start-n_pre_stim:beh_start]))
    if return_starts:
